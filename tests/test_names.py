"""Tests for multi_claude.names."""

from __future__ import annotations

from pathlib import Path

from multi_claude.names import NamesStore, default_path


def test_default_path_uses_xdg(monkeypatch) -> None:
    monkeypatch.setenv("XDG_CONFIG_HOME", "/tmp/xdg")
    assert default_path() == Path("/tmp/xdg/multi-claude/names.json")
...
def test_default_path_falls_back_to_home(monkeypatch) -> None:
    ...
    assert default_path() == Path("/home/test/.config/multi-claude/names.json")
